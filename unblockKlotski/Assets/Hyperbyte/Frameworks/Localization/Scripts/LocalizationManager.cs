...
namespace Hyperbyte.Localization
{
    ...
    public class LocalizationManager : Singleton<LocalizationManager>
    {
        ...
        [System.NonSerialized] public bool hasInitialized = false;
        ...
        Dictionary<string, string> englishTags = new Dictionary<string, string>();
...
#if UNITY_EDITOR
                PlayerPrefs.SetString("currentLang", "CN");
#endif
        ...
        public LocalizedLanguage GetCurrentLanguage()
        {
            ...
            if (!PlayerPrefs.HasKey("currentLang"))
            {
                if (localizationSettings.localizeToSystemDetectedLanguage)
                {
                    ...
                    {
#if UNITY_IOS
                        string iosname = iOSUtil.getCurIOSLang();
                        if (false==iosname.StartsWith("zh-Hans", StringComparison.Ordinal))
                        {
                            autoDetectLang = null;  //非简体中文
                        }                  
#endif
                    }
        ...
        void InitEnglishContent()
        {
            LocalizedLanguage defaultLanguage = allLocalizedLanaguages[localizationSettings.defaultLangauge];
            XDocument xDocLocalizedContent = XDocument.Parse(defaultLanguage.localizedTextFile.ToString());
            englishTags = new Dictionary<string, string>();

            var allElements = xDocLocalizedContent.Root.Elements("string");

            foreach (XElement ele in allElements)
            {
                englishTags.Add(ele.Attribute("name").Value, ele.Attribute("text").Value);
            }
        ...
        public string GetTextWithTag(string key)
        {
            ...
            {
                englishTags.TryGetValue(key, out val);
            }
