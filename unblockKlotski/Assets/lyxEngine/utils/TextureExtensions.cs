...
namespace VoxelBusters.Utility
{
	public static class TextureExtensions
	{
		...
		public static IEnumerator TakeScreenshot (System.Action<Texture2D> _onCompletionHandler)
		{
			...
			//根据屏幕缩放比来截图（算出起始坐标，宽度，高度）
			// Read pixel
			Rect rect=getMainRect();

            #if UNITY_EDITOR

			_texture	= new Texture2D((int)Mathf.Ceil(rect.width), (int)Mathf.Ceil(rect.height),TextureFormat.ARGB32, false);
#else
            _texture	= new Texture2D((int)Mathf.Ceil(rect.width), (int)Mathf.Ceil(rect.height),TextureFormat.RGB24, false);
#endif
